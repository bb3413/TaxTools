...
export class TaxForms {
	...
	static getForm(formname, index = 0) {
		...
		if (!formlist) {
			// Return undefined.
			// throw new Error(`TaxForms.getForm: Form ${formname} not found.`);
		} else {
			if (formlist.length === 0) {
				// Return undefined.
				// throw new Error(`TaxForms.getForm: No instances of form ${formname}.`);
			} else if ((formlist.length-1) < index) {
				throw new Error(`TaxForms.getForm: Index (${index}) is > last instance of form.`);
			} else {
	...
	static getTextValue(formname, ...lineno) {
		...
		Debug.enter(`TaxForms.getTextValue(${formname}, ${lineno})`);
		let str = "";
		let formlist = instances[formname];
		...
		Debug.exit(`TaxForms.getTextValue(${str})`);
		return str;
	}
	...
	static getValue(formname, ...lineno) {
		...
		Debug.enter(`TaxForms.getValue(${formname}, ${lineno})`);
		let sum = 0;
		let formlist = instances[formname];
		if (!formlist && create_on_demand.includes(formname)) {
			// Try to create; not an error if it fails; it may be a form that is not implemented yet.
		...
		Debug.exit(`TaxForms.getValue(${sum})`);
		return sum;
	}
